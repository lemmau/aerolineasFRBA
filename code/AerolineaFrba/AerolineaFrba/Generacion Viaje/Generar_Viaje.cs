...
namespace AerolineaFrba.Generacion_Viaje
{
    public partial class Generar_Viaje : Form
    {
        ...
        private void fechaSalida1_ValueChanged(object sender, EventArgs e)
        {
            ...
            label3.Visible = true;
            label4.Visible = true;
            fechaLlegada1.Visible = true;
            fechaLlegada.Visible = true;
            fechaEst1.Visible = true;
            fechaEst.Visible = true;
            using (var conn = DataAccess.GetConnection())
            {
                ...
                fechaSalida.Value = get_fecha_llegada();
                try
                {
                    conn.Open();
        ...
        private void aeronaves_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            tipo_servicio = aeronaves.Rows[e.RowIndex].Cells["tipoServicio1"].Value.ToString();
        ...
        {
            id_ruta_selec = Convert.ToInt32(rutas.Rows[e.RowIndex].Cells["id_ruta"].Value.ToString());
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            else if (this.id_ruta_selec == 0)
            {
                str_error += "Debe seleccionar una Ruta";
            }
            else if (fechaLLegadaEstimMenorFechaSalida())
            {
                str_error += "La fecha de llegada estimada no puede ser previa a la fecha de salida\n";
            }
            else if (fechaLLegadaEstimMayor24hs())
            {
                str_error += "El tiempo estimado de viaje no puede ser mayor a 24 hs\n";
            }
            if (!str_error.Equals(""))
            ...
            using (var conn = DataAccess.GetConnection())
            {
                ...
                var cmd = new SqlCommand("HAY_TABLA.alta_viaje", conn);
